...
fill_dhcp_request_options(dhcp_t *dhcp)
{
    ...
    len += fill_dhcp_option(&dhcp->bp_options[len], MESSAGE_TYPE_DHCP, &option, sizeof(option));
    req_ip = htonl(prev_dhcp->yiaddr);
    req_ip = ((req_ip>>24)&0xff) | // move byte 3 to byte 0
                    ((req_ip<<8)&0xff0000) | // move byte 1 to byte 2
                    ((req_ip>>8)&0xff00) | // move byte 2 to byte 1
                    ((req_ip<<24)&0xff000000); // byte 0 to byte 3
    len += fill_dhcp_option(&dhcp->bp_options[len], MESSAGE_TYPE_REQ_IP, (u_int8_t *)&req_ip, sizeof(req_ip));
    int first_byte = 5;
    server_ip = prev_dhcp->bp_options[first_byte+3]<<24 |
     prev_dhcp->bp_options[first_byte+2]<<16 |
      prev_dhcp->bp_options[first_byte+1]<<8 |
       prev_dhcp->bp_options[first_byte];
    len += fill_dhcp_option(&dhcp->bp_options[len], MESSAGE_TYPE_SERVER_IP, (u_int8_t *)&server_ip, sizeof(server_ip));
    option = 0;
