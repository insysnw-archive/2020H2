...
def read_varint(reader):
    ...
    for i in range(5):
        ...
        if not part & 0x80:
            return result
    raise IOError("Server sent a var int that was too big!")
...
def write_varint(writer, value):
    ...
    for _ in range(5):
        ...
        remaining >>= 7
    raise ValueError("The value %d is too big to send in a var int" % value)
...
data_types_by_name = {name.lower(): data_type for data_type in data_types for name in
                      data_type.aliases + [data_type.__name__]}
...
parser.add_argument('--repository', default='test.toml', help='configuration file path')
...
security_models = {3: SecurityModel}
...
class Packet:
    def __init__(self, sequence=None):
        if sequence == None:
            ...
            self.security_parameters = SecurityModel()
    ...
    def to_asn1(self):
        return SEQUENCE(
            [INTEGER(self.version, 1), self.header.to_asn1(), OCTET_STRING(self.security_parameters.encode()),
             self.data.to_asn1()])
...
while True:
    try:
        message, address = server_socket.recvfrom(4000)
