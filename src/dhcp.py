...
class Client:
    ...
    def set_data(self, data):
        ...
        if self.operation == "DHCPOFFER":
            ...
            print(f"!!!!!!!!! Отправляем REQUEST в овет на {self.operation} в бродкаст !!!!!!!!!")
            sock.sendto(pack, ("192.168.1.255", UDP_PORT_OUT))
        elif self.operation == "DHCPACK":
            print(f"!!!!!!!!! Получил DHCPACK, теперь {self.offerIP} мой на {self.leaseTime} секунд?! !!!!!!!!!")
...
if __name__ == '__main__':
    first_client = Client()
    sock.sendto(first_client.build_discover_pack(), ('192.168.1.255', UDP_PORT_OUT))
    print(f"!!!!!!!!! Посылаю DHCPDISCOVER в бродакст !!!!!!!!!")
    while True:
        recv_data, addr = sock.recvfrom(1024)
        print(f"!!!!!!!!! Получили пакет !!!!!!!!!")
